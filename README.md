...
Единственное, что **не трогается**, — режим модема: установщик по умолчанию его не менял (значит откатывать нечего), а перезапись NVM «на всякий случай» рискованна, поэтому NCM остаётся. Вернуть заводской MBIM — флагом `RESTORE_MBIM=1` (пишет NVM). Ещё есть `AUTO_REBOOT=0` — без перезагрузки.
