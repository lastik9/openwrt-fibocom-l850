...
- OpenWrt **25.x** (пакетный менеджер **apk**) или **24.x** (**opkg**) — под каждую ветку свой скрипт, см. «Установка».
...
В репозитории **два набора скриптов** — под OpenWrt 25 (`apk`) и под OpenWrt 24 (`opkg`). Сначала определи свою версию:
...
- есть **`apk`** → OpenWrt 25 → `install-fibocom-l850.sh`
- есть **`opkg`** → OpenWrt 24 → `install-fibocom-l850-owrt24.sh`

Скрипты сами проверяют пакетный менеджер и откажутся запускаться не на своей ветке, так что перепутать сложно.
...
Дальше всё одинаково: вопросы, флаги, поведение и итоговая настройка у обеих версий совпадают — отличается только пакетный менеджер и репозитории (для 24 это `.ipk`-фиды 132lan и 4IceG `Modem-extras`).
...
(на OpenWrt 24 — то же самое с именем своего скрипта: `DO_MODE_SWITCH=1 sh install-fibocom-l850-owrt24.sh`)
...
На **OpenWrt 24 (opkg)** — свой деинсталлятор:
...
**Отправка работает** (проверено на МегаФоне: и через панель sms-tool-js, и через
`sms_tool -d /dev/ttyACM0 send <номер> "текст"`). Отправка зависит от оператора и
состояния услуги на SIM: на тестовой Yota-симке сеть в какой-то момент начала
отвечать `Unidentified Subscriber` — это отказ **сети**, а не модема.

**Приём входящих SMS не работает.** Проверено на двух разных симках (в том числе
на свежей, с деньгами и заведомо рабочей услугой SMS), с отправкой с чужого
телефона.

**Характерный признак: отправляющий телефон показывает «не доставлено».** То есть
сеть даже не смогла доставить сообщение на устройство — отказ происходит **до**
модема. Поэтому настройками OpenWrt (`AT+CPMS`, `AT+CNMI`, `AT+CGSMS`) это не
лечится: чинить нечего, до модема сообщение не доходит.

При этом сообщения, уже лежащие в памяти SIM, модем читает нормально — панель их
показывает, `AT+CMGL` отдаёт. Механизм чтения исправен; отсутствует именно
доставка.

Причина: модем зарегистрирован для данных, но как получатель SMS сеть его не
видит — `AT+CIREG?` → `0,0` (IMS не поднят), `AT+CREG?` → `stat=6` («registered
for SMS only» через SGs). MT-SMS у операторов доставляются поверх **IMS**, а Intel
XMM7360 (L850-GL) в открытом стеке (proto `xmm`, NCM) IMS в дата-режиме не
поднимает.

Что проверяли (без результата): `AT+CGSMS=1` (домен CS/SGs — и так был выставлен),
`AT+CPMS="SM"`/`"ME"` (перебор хранилищ; `MT` этот модем не поддерживает),
`AT+CNMI=2,1,0,0,0` (индикация новых сообщений), loopback-отправку самому себе,
отправку с чужого номера на здоровую SIM.

Если приём SMS для тебя критичен — не рассчитывай на этот модуль; L860-GL ведёт
себя иначе.
