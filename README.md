...
### SMS: отправка работает, приём — получить не удалось
...
**Приём новых SMS в наших тестах получить не удалось.** При этом сообщения, уже
лежащие в памяти SIM, модем читает нормально — панель их показывает, `AT+CMGL`
отдаёт. То есть механизм чтения исправен; не доходит именно доставка новых
сообщений.

Вероятная причина: MT-SMS (входящие) у операторов доставляются поверх **IMS**
(`AT+CIREG?` → `0,0` — IMS не зарегистрирован), а Intel XMM7360 (L850-GL) в
открытом стеке (proto `xmm`, NCM) IMS в дата-режиме не поднимает. Оговорка:
исчерпывающего теста (входящее с чужого номера на заведомо «здоровой» SIM) мы не
проводили, так что нельзя исключать, что на другом операторе/тарифе приём
заработает.
...
(`send` проходит с референс-номером, но счётчик `AT+CPMS?` не растёт).
