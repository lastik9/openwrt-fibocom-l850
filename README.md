...
При запуске скрипт задаёт два вопроса — **APN** (по умолчанию `internet`) и **ставить ли русские локали** панелей (`[Y/n]`), — после чего работает сам. Оба можно задать заранее переменными и пропустить вопрос: `APN=internet.yota` и `INSTALL_RU=no` (или `yes`).
